...
for (i in 1:length(yahoo_a1)){
  data <- yahoo_a1[[i]]
  result_81_fbiad_yh_a1[[i]] <- run_nexus(model=model_fbiad, data=data, warm_size=wm_size[5], batch_size=bt_size[5], mem_batches=0, png_folder="dev/plots/")
}

names(result_81_fbiad_yh_a1) <- names(yahoo_a1)
save(result_81_fbiad_yh_a1, file="develop/result_81_fbiad_yh_a1_complete.RData", compress = TRUE)
